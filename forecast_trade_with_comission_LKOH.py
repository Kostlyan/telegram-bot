...
df1 = read_csv('LKOH.csv')  # заливаем csv файл с коировками, скаченными с finam
#df_main = read_csv('LKOH_Forecast_Test.csv')
ticker_code = 'LKOH'
...
nn = MLPClassifier(hidden_layer_sizes=(72, 68), max_iter=20000, alpha=0.00001, random_state=0)  # задаем модель

pickle_in = open('Luk_3.pickle','rb')
...
df2.to_csv('LKOH_Forecast_Test.csv')
deals.to_csv('deals_lkoh.csv')
now = datetime.datetime.now()
#df2.to_csv('/opt/python/archive/LKOH_Forecast_Test'+str(now.year)+str(now.month)+str(now.day)+str(now.hour)+str(now.minute)+str(now.second)+'.csv')
